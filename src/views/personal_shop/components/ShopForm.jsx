...
const FormSchema = Yup.object().shape({
  name: Yup.string()
    ...
    .max(60, 'Shop name must only be less than 60 characters.'),
  brand: Yup.string()
  ...
  keywords: Yup.array()
    .of(Yup.string())
    .min(1, 'Please enter at least 1 keyword for this shop.'),
  sizes: Yup.array()
    .of(Yup.number())
    .min(1, 'Please enter a size for this shop.'),
  isFeatured: Yup.boolean(),
  isRecommended: Yup.boolean(),
  availableColors: Yup.array()
    .of(Yup.string().required())
    .min(1, 'Please add a default color for this shop.')
});

const ShopForm = ({ shop, onSubmit, isLoading }) => {
  const initFormikValues = {
    name: shop?.name || '',
    brand: shop?.brand || '',
    price: shop?.price || 0,
    maxQuantity: shop?.maxQuantity || 0,
  ...
  const {
    ...
    onFileChange,
    removeImage
  } = useFileHandler({ image: {}, imageCollection: shop?.imageCollection || [] });
  ...
  const onSubmitForm = (form) => {
    onSubmit({
      ...
      // image: imageFile?.image?.file || shop.imageUrl,
            ...
            <div className="product-form-inputs">
              ...
              <div className="product-form-field product-form-submit">
                ...
                >
                  ...
                  &nbsp;
                  {isLoading ? 'Saving Shop' : 'Save Shop'}
                </button>
              </div>
            </div>
            {/* ----THUBMNAIL ---- */}
